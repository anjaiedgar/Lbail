...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (
	    ...
	    await authState.keys.transaction(async () => {
		    ...
		    if (isGroupOrStatus &&!isRetryResend) {
	    		...
	    		// Fix: compute this before the device loop (and before `user` gets shadowed
	    		// by the loop variable) so noSelfSync can safely take self-chat into account.
	    		...
	    		for (const { user, jid } of devices) {
                    ...
                    let ptcp = false
                    // Fix: don't let noSelfSync strip out every recipient when the chat is
                    // with ourselves (e.g. "Message Yourself") — that would silently drop
                    // the message with zero devices to deliver to.
                    if (noSelfSync && !isSelfChatDestination) {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const luki = new imup(Utils_1, waUploadToServer, relayMessage, options.brand)
